...
const Adventure = () => {
  ...
  useEffect(() => {
    booklibrarydetails.getBooks("adventure")
      .then(response => {
        const adventureBooks = response.data.filter(book =>
          book.genre.toLowerCase() === "adventure"
        );
        setBooks(adventureBooks);
      })
      .catch(error => {
  ...
  return (
    <div className="scifi-container">
      <h1 className="scifi-title">Adventure Books</h1>
      <button onClick={() => navigate("/")} className="scifi-back-button">
        Back
      </button>
      {books.length === 0 ? (
        <p>No adventure books found.</p>
      ) : (
        books.map(book => (
